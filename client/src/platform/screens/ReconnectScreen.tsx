...
export function ReconnectScreen({ onBack }: ReconnectScreenProps) {
  ...
  const seatsMatchRoom = reconnectableSeatsRoomCode === roomCode;
  const lookupMatchesRoom = seatLookupState.roomCode === roomCode;
  ...
  const submitClaim = () => {
    ...
    requestSeatClaim(roomCode, selectedPlayerId, claimantName.trim());
  };
  ...
  return (
    <div className="reconnect-screen home-actions">
      <div className="reconnect-screen-heading">
        ...
        <h2 id="seat-recovery-title">Восстановление места</h2>
