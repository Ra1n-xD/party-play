...
export function bindPlayerSocket(
  ...
  if (player.kicked || player.owner.kind !== "human") {
    return { ok: false, error: "Место закрыто", code: "SEAT_CLOSED" };
  }
  if (player.controller.kind === "human" && player.controller.socketId !== socketId) {
    return {
      ok: false,
  ...
  const nextEpoch = player.controller.epoch + 1;
  player.socketId = socketId;
...
function isReconnectablePlayer(player: Player | undefined): player is Player {
  return !!player && player.owner.kind === "human" && !player.connected && !player.kicked;
}
...
export function createSeatClaim(
  ...
  if (!module || module.lifecycle(room) === "results") {
...
function commitHostTransfer(
  ...
  touchRoom(room.code);
  const module = getServerGameModule(room.gameId);
  module?.setAdminPause(room, false, io, false);
...
export function kickPlayerPermanently(
  ...
  const module = getServerGameModule(room.gameId);
  if (!module) {
    return { success: false, error: "Игра недоступна" };
  }
  if (module.lifecycle(room) === "results") {
    return { success: false, error: "Игра завершена" };
  }
  if (playerId === actorId) {
  ...
  if (player.owner.kind === "bot")
    return { success: false, error: "Бота нельзя удалить этой командой" };
  ...
  const exclusion = module.excludeSeat(room, player.id, io);
  if (!exclusion.success) {
    return {
      success: false,
      error: exclusion.error ?? "Не удалось удалить игрока",
    };
  }

  touchRoom(room.code);
  player.sessionToken = generateSessionToken();
  if (player.owner.kind === "human") {
    player.owner.sessionToken = player.sessionToken;
  }
  cancelClaimsForPlayer(room, player.id, io, "Место закрыто администратором");
  let needsFinalPublish = false;
  if (player.controller.kind !== "none") {
    player.controller = { kind: "none", epoch: player.controller.epoch + 1 };
    needsFinalPublish = true;
  }
  if (!player.kicked || player.connected || player.alive) {
    needsFinalPublish = true;
  }
  player.kicked = true;
  player.isBot = false;
...
export type AssignTemporaryBotResult = { success: true } | { success: false; error: string };

export function assignTemporaryBot(
  room: Room,
  actorId: string,
  playerId: string,
  io: IOServer,
): AssignTemporaryBotResult {
  const actor = room.players.get(actorId);
  if (room.hostId !== actorId || !isEligibleHost(actor)) {
    return { success: false, error: "Только хост может выполнить это действие" };
  }
  const player = room.players.get(playerId);
  if (!player || player.owner.kind !== "human" || player.kicked) {
    return { success: false, error: "Игрок не найден" };
  }
  const module = getServerGameModule(room.gameId);
  if (!module || module.lifecycle(room) !== "playing") {
    return { success: false, error: "Временный бот доступен только во время игры" };
  }
  if (player.connected || player.controller.kind === "human") {
    return { success: false, error: "Игрок уже подключён" };
  }
  if (player.controller.kind === "bot" && player.controller.temporary) {
    return { success: true };
  }
  ...
  module.onControllerChanged(room, player.id, "temporary-bot", io);
  module.publish(room, io);
  return { success: true };
}
