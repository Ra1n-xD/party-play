...
import { Player, getAlivePlayers, touchRoom } from "../../platform/roomManager.js";
import type { BunkerRoom as Room } from "./runtime.js";
  ...
  if (room.gameState) {
    ...
    const timer = setTimeout(() => {
      void executeInRoom(room.code, () => {
        if (!room.gameState || room.gameState.phaseTimer !== timer) return;
        if (isGameplayPaused(room)) return;
    ...
    room.gameState.phaseTimer = timer;
  }
}
...
export function adminShuffleAll(
  ...
  if (attributeType === "action") {
    ...
    const attrs: { playerIdx: number; attrIdx: number; attr: Attribute }[] = [];
    for (const player of alivePlayers) {
...
export function addDisconnectPause(
  ...
  const reasons = room.pauseReasons.disconnectedSeatIds;
...
export function removeDisconnectPause(
  ...
  const gs = room.gameState;
  if (!gs || !room.pauseReasons.disconnectedSeatIds.delete(playerId)) return;
...
export function setAdminPause(
  ...
  if (room.pauseReasons.admin === paused) return { success: true, error: "" };

  const wasPaused = isGameplayPaused(room);
  room.pauseReasons.admin = paused;
...
function getPauseKind(room: Room): PauseKind {
  const reasons = room.pauseReasons;
  const hasReconnectPause = reasons.disconnectedSeatIds.size > 0;
...
export function buildPublicState(room: Room): PublicGameState {
  ...
  const players: PlayerInfo[] = Array.from(room.players.values()).map((p) => {
    ...
    const info: PlayerInfo = {
      ...
      name: p.name,
      ready: p.ready,
      connected: p.controller.kind !== "none",
      alive: p.alive,
  ...
  return {
    ...
    disconnectedPlayerIds: Array.from(room.pauseReasons.disconnectedSeatIds),
    spectatorCount: room.spectators.size,
  };
}

export function buildPrivateState(room: Room, seatId: string): BunkerPrivateState | null {
  const player = room.players.get(seatId);
  if (!player || player.kicked || player.owner.kind !== "human") return null;
  const voterStatus: BunkerVoterStatus | null = room.gameState
    ? {
        phase: room.gameState.phase,
        roundNumber: room.gameState.roundNumber,
        currentVotingInRound: room.gameState.currentVotingInRound,
...
export function broadcastState(room: Room, io: IOServer): void {
  ...
  publishRoomSnapshots(room, io, {
    ...
    publicProjection: buildPublicState,
    privateProjection: buildPrivateState,
  });
  for (const player of room.players.values()) {
    if (player.owner.kind !== "human" || player.controller.kind !== "human" || !player.socketId) {
      continue;
    }
    const privateState = buildPrivateState(room, player.id);
    if (privateState) io.to(player.socketId).emit("game:private", privateState);
  }
